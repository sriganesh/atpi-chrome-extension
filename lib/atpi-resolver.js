...
const atpiResolver = (() => {
  ...
  async function resolveHandle(handle, timeout) {
    ...
    try {
      ...
      if (typeof console !== 'undefined' && console.log) {
        console.log(`[ATPI Resolver] Handle resolution took ${Date.now() - startTime}ms for ${handle}`);
      }
      return result.did;
    } catch (error) {
  ...
  async function resolveLocal(url, options = {}) {
    ...
    if (identifier.type === 'handle') {
      ...
      if (typeof console !== 'undefined' && console.log) {
        console.log(`[ATPI Resolver] Handle->DID took ${Date.now() - handleStartTime}ms`);
      }
    }
    ...
    if (typeof console !== 'undefined' && console.log) {
      console.log(`[ATPI Resolver] DID->Endpoint took ${Date.now() - didStartTime}ms`);
    }
    ...
    if (typeof console !== 'undefined' && console.log) {
      console.log(`[ATPI Resolver] XRPC fetch took ${Date.now() - fetchStartTime}ms`);
    }
